...
- 分布式系统设计存在哪些取舍？
- 不同分布式系统的理论限制有哪些？
- 各种一致性模型的区别是什么？对系统有何影响？
- 分布式事务和一致性的关系是什么？和共识的关系又是什么？

对于需要工程实践经验的工程师来说，最重要的问题是：
>实现一个分布式系统，究竟有哪些难点和相应的解决思路？

本文是作者通过实现一个分布式缓存，对以上部分问题进行的一次梳理，和大家共同交流。正所谓：
...
该缓存服务除了实现常见的"put _key_ _value_"、"get _key_"、"del _key0_  \[_key1_ ...\]"操作，
...
### skip list的持久化实现
在内存中实现skip list的数据结构非常简单，使用文件接口的持久化实现则需要考虑很多细节。
