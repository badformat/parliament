...
## 前言
如果一个系统提供服务时，需要多台机器参与，这类系统都可以被称为分布式系统。

存在多台机器的原因有三类：

- 系统各个模块的所有者、运行环境、架构体系、发布频率等客观条件不同，需要多台机器。
- 单机无法满足性能或容量要求，需要多台机器进行横向扩展。
- 基于副本（replicate）的容错设计。

一个大型系统，这三个原因都会存在。

在一些文档中，容错经常和高可用、灾备混淆。可以通过例子简单的[区分](http://www.ruanyifeng.com/blog/2019/11/fault-tolerance.html)：

- 容错：系统允许某些错误发生，并且保证在修复前，用户功能不受影响。如大型客机的引擎。
- 高可用：系统发生错误后，可以快速恢复。如汽车的备胎。
